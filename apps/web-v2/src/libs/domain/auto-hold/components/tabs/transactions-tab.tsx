...
const generateCardHistory = (cardF6: string, cardL4: string, baseMid: string): CardHistoryTransaction[] => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (Math.random() < 0.7) {
      ...
      const randomIndex = Math.floor(Math.random() * (midVariations.length - 1)) + 1;
            ...
            >
              ...
              <Dialog.Body flex={1} overflowY="auto" p={0}>
                {selectedCard && (() => {
                  ...
                  // Apply sorting
                  const currentMid = transactions[0]?.mid || '';
                  if (cardHistorySortColumn === 'mid') {
                    ...
                    filteredHistory.sort((a, b) => {
                      const aIsCurrentMid = a.mid === currentMid;
                      const bIsCurrentMid = b.mid === currentMid;
