...
export default function MCCCodeTable(
  props: MCCCodeTableProps
): React.JSX.Element {
  const { mcc } = props;
  ...
  const handleFormSubmit = async (
    values: components['schemas']['RiskRuleWhiteListMccEntity']
  ) => {
    try {
      await mutateAsync({
        body: {
      ...
      toaster.success({
        title: 'Updated mcc code successfully',
      });
    } catch (error) {
      ...
      <Flex justify="space-between" align="center" mb={4}>
        <Box>
          ...
          <Text fontSize="sm" color="gray.600">
            Last updated: {new Date(mcc.lastUpdatedDate).toLocaleString()} by{' '}
            <Text as="span" fontWeight="bold">
              {mcc.lastUpdatedBy || 'Unknown'}
            </Text>
          </Text>
        </Box>
